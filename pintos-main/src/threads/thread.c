...
static bool
thread_tick_less (const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
...
/** Blocks the current thread to sleep for `ticks` global ticks */
void
thread_sleep_until (int64_t ticks)
{
    ...
    printf("a thread was called to sleep with %lld ticks\n", ticks);
    old_level = intr_disable ();
    if (cur != idle_thread)
        list_insert_ordered(&sleep_list, &cur->elem, thread_tick_less, NULL);
